...
export default function ClientInvoicesPage() {
  ...
  const [rcvForm, setRcvForm] = useState({
    invoiceNumber: "", vendorName: "", subject: "",
    ...
    <div className="space-y-4 animate-fade-in">
                ...
                <tbody>
                  {invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-navy-50 border-b border-navy-100 last:border-0">
                      ...
                      <td className="px-4 py-3"><StatusBadge status={inv.status} role="CLIENT" /></td>
      ...
      {pageTab === "received" && (
        <>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* 担当者フィルター */}
            <select value={rcvFilterUserId} onChange={e => setRcvFilterUserId(e.target.value)}
              className="px-2 py-1.5 text-[12px] border border-navy-200 rounded-lg text-navy-700 bg-white focus:outline-none focus:ring-1 focus:ring-navy-400">
              <option value="">担当者：全員</option>
              {allUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
          ...
          <div className="bg-white rounded-xl border border-navy-200 p-6 w-[480px] shadow-xl">
            ...
            <div className="space-y-3">
              ...
              {rcvForm.amount && (
                <div className="flex gap-4 px-3 py-2 bg-navy-50 rounded-lg text-[11px] text-navy-600 tabular-nums">
                  {(() => {
                    ...
                    const tax = inc - ex
                    return (<><span>税込：¥{inc.toLocaleString("ja-JP")}</span><span className="text-navy-400">／</span><span>税抜：¥{ex.toLocaleString("ja-JP")}</span><span className="text-navy-400">／</span><span>消費税：¥{tax.toLocaleString("ja-JP")}</span></>)
                  })()}
                </div>
              )}
              <div>
                ...
                <select className="w-full px-3 py-2 border border-navy-200 rounded-lg text-[13px] bg-white"
                  value={rcvForm.assignedUserId} onChange={e => setRcvForm(f => ({ ...f, assignedUserId: e.target.value }))}>
              ...
              <div>
                ...
                <FileDropZone onFile={f => setRcvModalFile(f)} currentFileName={rcvModalFile?.name} compact />
                {rcvModalFile && (
                  <button type="button" onClick={() => setRcvModalFile(null)} className="mt-1 text-[10px] text-red-400 hover:text-red-600">選択を解除</button>
                )}
              </div>
              <div>
      ...
      {showSendModal && targetRcv && (
        <div className="fixed inset-0 bg-navy-900/40 z-50 flex items-center justify-center"
          ...
          <div className="bg-white rounded-xl border border-navy-200 p-6 w-[380px] shadow-xl">
